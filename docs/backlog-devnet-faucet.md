...
sequenceDiagram
    ...
    B->>C: GET /v2/state/active-contracts (fetch AmuletRules)
    C-->>B: AmuletRules + blob
    B->>C: GET /v2/state/active-contracts (fetch OpenMiningRound)
    C-->>B: OpenMiningRound + blob
    B->>C: POST /v2/interactive-submission/prepare
    C-->>B: preparedTx + hash
    B-->>W: preparedTxHash

    Note over W: wallet signs locally<br/>with user's private key
