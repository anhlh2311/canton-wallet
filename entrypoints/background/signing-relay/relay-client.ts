/**
 * Signing relay Socket.io client for the Canton Wallet extension service worker.
 *
...
class SigningRelayClient {
  ...
  setAutoApprove(enabled: boolean): void {
    ...
    console.log(`[Canton Wallet Relay] Auto-approve ${enabled ? 'enabled' : 'disabled'}`);
  }
  ...
  connect(relayUrl: string, partyId: string, options?: { authToken?: string; apiKey?: string }): void {
    ...
    this.socket.on('connect', () => {
      console.log('[Canton Wallet Relay] Connected to signing relay');
    ...
    this.socket.on('disconnect', (reason) => {
      console.log('[Canton Wallet Relay] Disconnected:', reason);
      chrome.alarms.clear(KEEPALIVE_ALARM);
    });

    this.socket.on('connect_error', (err) => {
      console.warn('[Canton Wallet Relay] Connection error:', err.message);
    });
  }
  ...
  registerKeys(keys: RegisterKey[]): void {
    ...
    console.log(`[Canton Wallet Relay] Registered ${keys.length} key(s)`);
  }
  ...
  private async handleSignRequest(request: SignRequest): Promise<void> {
    try {
      ...
      if (this._autoApprove) {
        console.log('[Canton Wallet Relay] Auto-approved sign request (onboarding mode)');
        approved = true;
      } else {
      ...
      console.error('[Canton Wallet Relay] Sign request failed:', error);
      this.sendSignResponse({
        txId: request.txId,
        status: 'failed',
        signature: null,
