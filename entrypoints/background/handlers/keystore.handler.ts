  ...
  try {
    ...
    if (partyStatus !== 'SUCCESSFULLY') {
      ...
      try {
        ...
        const partyHint = import.meta.env.VITE_PARTY_HINT || 'canton-wallet';
        console.log(`[Canton Wallet] Calling createWallet with partyHint: ${partyHint}`);
        ...
        console.log(`[Canton Wallet] createWallet result: ${result.wallet.partyId} ${result.wallet.status}`);
        ...
        console.log('[Canton Wallet] Registered party with dapp-core backend');
